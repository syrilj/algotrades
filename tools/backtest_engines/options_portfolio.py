...
def run_options_backtest(
    ...
    # Generate trade signals once (legacy interface) or per-day (stateful generate_day)
    has_day = callable(getattr(engine, "generate_day", None))
    if not has_day:
    ...
    for current_date in dates:
        ...
        # 3. Get today's signals (legacy batch or stateful per-day)
        if has_day:
            state.update({
                "cash": cash,
                "positions": [
            ...
            day_signals = engine.generate_day(data_map, state, ts)
        else:
