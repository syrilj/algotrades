...
class SignalEngine:
    """v20b: v16 meta+Kelly + XLP/SPY defensive block + drop ARM. No Mag7 hard gate (capacity)."""
    ...
    def _signals_on_frame(self, data, cfg, code: str):
        ...
        vol_z_20 = (data["volume"] - _sma(data["volume"], 20)) / data["volume"].rolling(20, min_periods=5).std()
        vol_z_20 = vol_z_20.replace([np.inf, -np.inf], np.nan).fillna(0.0)
        # Vol boost for all symbols (helps IONQ/APLD most, adds conviction)
        vol_z_boost = np.clip((vol_z_20 - 1.5) / 2.5, 0, 0.3)
        ...
        for i in range(len(data)):
            ...
            if not in_pos:
                # SPY/TSLA require vol_z threshold for entry quality (they have lower WR)
                if code in ("SPY.US", "TSLA.US"):
                    vol_z_thresholds = {"SPY.US": 1.8, "TSLA.US": 1.6}
                    vol_th = vol_z_thresholds[code]
                    vol_z_ok = vol_z_20.iloc[i] >= vol_th
                    if not vol_z_ok:
                        continue
                if bool(long_entry.iloc[i]):
                    ...
                    # Volume boost: add up to 30% confidence when vol_z >= 1.5
                    boosted_proba = min(1.0, proba + float(vol_z_boost.iloc[i]))
                    meta_sz=_prob_to_size(boosted_proba, self._thr)
