...
class SignalEngine:
    ...
    def _signals_on_frame(self, data, cfg, code: str):
        ...
        # --- Structure gates (desk logic A/B): volume first, EMA200 structure ---
        # Hypothesis: block dry-volume + below-200 entries → better Sharpe/PF/DD
        ema22=_ema(close,22); ema200=_ema(close,200)
        above_200=(close>=ema200)&ema200.notna()
        vol=data["volume"]; vsma=_sma(vol,vol_sma)
        rvol=(vol/vsma.replace(0,np.nan)).replace([np.inf,-np.inf],np.nan).fillna(0.0)
        vol_awake=(rvol>=1.0)|vp["vol_expand"]|vp["confirm_up"]
        vol_dry=vp["red_flag_up"]|((rvol<0.75)&(vol<=vol.shift(vol_look)))
        atr_safe=atr.replace(0,np.nan)
        dist_22=((close-ema22)/atr_safe).replace([np.inf,-np.inf],np.nan)
        near_22=above_200&(dist_22>=-0.15)&(dist_22<=0.55)
        # Keep classic in-value entries; also allow near-22 pullbacks that pass hard gates base
        structure_ok=above_200&(~vol_dry)&vol_awake
        long_entry=long_entry&structure_ok
        # Soft boost path: if near 22 with healthy dip volume, don't require rvol>=1 (awake via healthy_pull)
        soft_22=near_22&vp["healthy_pull"]&above_200&(~vp["red_flag_up"])&long_hard
        long_entry=long_entry|soft_22
