...
# v18 dual sleeve — from parallel feedback loops (runs/poc_va_multiloop)
# Sleeve A sniper: APLD/IONQ — QQQ trend + local MACD-HA green (+ red-flag / vol expand)
# Sleeve B large:  TSLA/MU — QQQ+Mag7 breadth gate + red-flag block
# Proof: trade-level OOS lifts in SYNTHESIS.json — engine backtest still required to promote.

TRADE_UNIVERSE = {"APLD.US", "IONQ.US", "TSLA.US", "MU.US"}
REGIME_REF = {"QQQ.US"}
MAG7 = ["AAPL.US", "MSFT.US", "NVDA.US", "GOOGL.US", "AMZN.US", "META.US", "TSLA.US"]

SNIPER = {"APLD.US", "IONQ.US"}
LARGE = {"TSLA.US", "MU.US"}
REGIME_APPLY_TO = TRADE_UNIVERSE
# per-sleeve regime selector applied in generate()
SLEEVE_REGIME = {
    "APLD.US": "gate_qqq_trend",
    "IONQ.US": "gate_qqq_trend",
    "TSLA.US": "gate_qqq_and_mag7",
    "MU.US": "gate_qqq_and_mag7",
}
...
def _mag7_breadth_ge4(data_map: Dict[str, pd.DataFrame], ref_index) -> pd.Series:
    parts = []
    for code in MAG7:
        if code not in data_map:
            continue
        d = data_map[code].copy()
        ...
        sma = daily["close"].rolling(20, min_periods=20).mean()
        above = (daily["close"] > sma).shift(1).fillna(False).astype(bool)
        parts.append(above.reindex(ref_index).ffill().fillna(False))
    if not parts:
        return pd.Series(False, index=ref_index)
    breadth = pd.concat(parts, axis=1).sum(axis=1)
    return (breadth >= 4).astype(bool)
...
class SignalEngine:
    """v18 dual sleeve from multiloop findings.

    Sniper (APLD/IONQ): QQQ trend + local MACD-HA green + red-flag block + vol expand.
    Large (TSLA/MU): QQQ∧Mag7≥4 + red-flag block (no forced vol-expand; DNA differs).
    """
    ...
    def _signals_on_frame(self, data, cfg, regime_ok=None, sleeve: str = "sniper"):
        ...
        # Sleeve-specific hard filters (from multiloop)
        long_entry = long_entry & (~vp["red_flag_up"].fillna(False).astype(bool))
        local_green = local_ha["ha_green"].fillna(False).astype(bool)
        if sleeve == "sniper":
            # Combo winner: qqq_trend + local_macd_green (trade-level 100%/100% n=11)
            long_entry = long_entry & local_green
            long_entry = long_entry & vol_expand.fillna(False)
        else:
            # Large: Mag7 regime already applied; optional vol expand from DNA
            if require_volume_expand:
                long_entry = long_entry & vol_expand.fillna(False)
            # prefer price above SMA20 proxy via close>prior 20-bar SMA on signal TF
            sma20 = close.rolling(20, min_periods=20).mean().shift(1)
            above_sma = (close > sma20).fillna(False)
            long_entry = long_entry & above_sma
    ...
    def _one(self, code, df, regime_ok=None):
        cfg = self._cfg(code)
        sleeve = "sniper" if code in SNIPER else "large"
        data = df.copy()
        ...
        if signal_tf:
            ...
            return self._signals_on_frame(frame, cfg, regime_ok=reg_f, sleeve=sleeve).reindex(
                data.index, method="ffill"
            ).fillna(0.0)
        return self._signals_on_frame(data, cfg, regime_ok=reg, sleeve=sleeve)
    ...
    def generate(self, data_map: Dict[str, pd.DataFrame]) -> Dict[str, pd.Series]:
        ...
        if "QQQ.US" in data_map:
            ...
            qqq_trend = _qqq_trend_ok(daily)
            mag7 = _mag7_breadth_ge4(data_map, daily.index)
            # align qqq green component for qqq_and_mag7 (htf green only, not full trend)
            c = daily["close"].astype(float)
            macd = _ema(c, 12) - _ema(c, 26)
            hist = macd - _ema(macd, 9)
            qqq_green = ((hist > 0) & (hist > hist.shift(1))).shift(1).fillna(False).astype(bool)
            regimes["gate_qqq_trend"] = qqq_trend
            regimes["gate_qqq_and_mag7"] = (qqq_green & mag7).astype(bool)
        ...
        for code, df in data_map.items():
            if code not in TRADE_UNIVERSE:
                ...
                continue
            gate_name = SLEEVE_REGIME.get(code, "gate_qqq_trend")
            regime_ok = regimes.get(gate_name)
