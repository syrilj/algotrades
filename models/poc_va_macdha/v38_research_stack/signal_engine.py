...
def volume_price_state(df, look=5, vol_sma=20):
    """Volume-price confirmation / red-flag + light Coulling VPA (v17b research).
    ...
    no_demand:    price↑ + low volume (soft veto only — never hard block)
    stopping_reclaim: recent absorption then price up (commitment)
    """
    close = df["close"]
    high = df["high"]
    low = df["low"]
    ...
    commitment = (confirm_up | stopping_reclaim).fillna(False)
...
def _soft_feature_mult(
    ...
    stopping_reclaim: bool = False,
    high_beta_qqq_weak: bool = False,
) -> float:
    """Soft size mult — research stack. Never hard-blocks.

    Sources:
    - v36 feature evolve: atr/room/macd/ret_5d
    - v32/v35 soft structure: good ×1.15, weak/chase ×0.55
    - v17b book VPA light: no_demand soft veto, commitment/stopping boost
    ...
    if high_beta_qqq_weak:
        m *= 0.70
    return float(np.clip(m, 0.35, 1.35))
...
class SignalEngine:
    """v38_research_stack: research fusion on frozen primary SIDE.

    Stack (all secondary — no new hard entry ANDs):
      v23/v37 primary specialists + XLP macro
      v37 continuous soft meta size + half-Kelly
      v36 genome: risk scale, after-win/loss feedback
      v36/v32 soft structure mults
      v17b soft VPA (no_demand cut, commitment/stopping boost)
    ...
    def _signals_on_frame(self, data, cfg, code: str, data_map: Dict[str, pd.DataFrame]):
        ...
        no_demand = vp["no_demand"].shift(1).fillna(False).astype(bool)
        commitment = vp["commitment"].shift(1).fillna(False).astype(bool)
        stopping_reclaim = vp["stopping_reclaim"].shift(1).fillna(False).astype(bool)
            ...
            if not in_pos:
                if bool(long_entry.iloc[i]):
                    ...
                    feat_m = _soft_feature_mult(
                        ...
                        bool(no_demand.iloc[i]),
                        bool(commitment.iloc[i]),
                        bool(stopping_reclaim.iloc[i]),
                        bool(hb_soft),
                    )
