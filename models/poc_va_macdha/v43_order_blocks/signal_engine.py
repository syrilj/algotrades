...
def calculate_lux_order_blocks(df: pd.DataFrame, length: int = 5, mitigation_method: str = "Wick") -> pd.DataFrame:
    df = df.copy()
    n = len(df)
    
    vol = df["volume"].to_numpy(dtype=float)
    phv = np.zeros(n, dtype=bool)
    
    for i in range(2 * length, n):
        mid_idx = i - length
        val_mid = vol[mid_idx]
        left_max = np.max(vol[mid_idx - length : mid_idx])
        right_max = np.max(vol[mid_idx + 1 : i + 1])
        if val_mid > left_max and val_mid > right_max:
            phv[i] = True
    ...
    for i in range(length, n):
        mid_idx = i - length
        if high[mid_idx] > upper[i]:
            current_os = 0
        elif low[mid_idx] < lower[i]:
            current_os = 1
        os_state[i] = current_os
        
    target_bull = np.zeros(n, dtype=float)
    target_bear = np.zeros(n, dtype=float)
    if mitigation_method == "Close":
        for i in range(length - 1, n):
            target_bull[i] = np.min(close[i - length + 1 : i + 1])
            target_bear[i] = np.max(close[i - length + 1 : i + 1])
    else:
        for i in range(length - 1, n):
            target_bull[i] = np.min(low[i - length + 1 : i + 1])
            target_bear[i] = np.max(high[i - length + 1 : i + 1])
            
    active_bull_obs_series = []
    active_bear_obs_series = []
    
    bull_obs = []
    bear_obs = []
    
    bull_ob_val = np.full(n, np.nan)
    bear_ob_val = np.full(n, np.nan)
    hl2 = (high + low) / 2.0
    
    for i in range(n):
        mid_idx = i - length
        
        if phv[i] and mid_idx >= 0:
            if os_state[i] == 1:
                top = hl2[mid_idx]
                btm = low[mid_idx]
                avg = (top + btm) / 2.0
                bull_obs.append({'top': top, 'btm': btm, 'avg': avg, 'left_idx': mid_idx, 'idx': i})
                bull_ob_val[i] = low[mid_idx]
            elif os_state[i] == 0:
                top = high[mid_idx]
                btm = hl2[mid_idx]
                avg = (top + btm) / 2.0
                bear_obs.append({'top': top, 'btm': btm, 'avg': avg, 'left_idx': mid_idx, 'idx': i})
                bear_ob_val[i] = high[mid_idx]
                
        t_bull = target_bull[i]
        bull_obs = [ob for ob in bull_obs if not (t_bull < ob['btm'])]
        
        t_bear = target_bear[i]
        bear_obs = [ob for ob in bear_obs if not (t_bear > ob['top'])]
        
        active_bull_obs_series.append(list(bull_obs))
        active_bear_obs_series.append(list(bear_obs))
        
    return pd.DataFrame({
        "active_bull_obs": active_bull_obs_series,
        "active_bear_obs": active_bear_obs_series,
        "os_state": os_state,
        "phv": phv
    }, index=df.index)
...
class SignalEngine:
    """v43_order_blocks: v39d baseline + LuxAlgo Order Blocks mapping.
    ...
    def __init__(self):
        ...
        try:
            ...
            self._live_streak = float(_la.size_mult_for("v43_order_blocks") or 1.0)
    ...
    def _signals_on_frame(self, data, cfg, code: str, data_map: Dict[str, pd.DataFrame]):
        ...
        ob_length=int(cfg.get("ob_length",5))
        ob_mitigation=cfg.get("ob_mitigation","Wick")
        ob_df=calculate_lux_order_blocks(data, ob_length, ob_mitigation)
        active_bull_obs=ob_df["active_bull_obs"]
        active_bear_obs=ob_df["active_bear_obs"]
        
        levels=_prior_session_profile(data,profile_lookback,profile_rows,value_area_pct)
        fallback_poc=levels["poc"]
        fallback_vah=levels["vah"]
        fallback_val=levels["val"]
        
        poc=pd.Series(np.nan, index=data.index)
        vah=pd.Series(np.nan, index=data.index)
        val=pd.Series(np.nan, index=data.index)
        close=data["close"]
        
        for idx in range(len(data)):
            px=float(close.iloc[idx])
            bulls=active_bull_obs.iloc[idx]
            under_bulls=[ob for ob in bulls if ob["btm"]<=px]
            if under_bulls:
                nearest_bull=max(under_bulls, key=lambda x: x["btm"])
                val.iloc[idx]=nearest_bull["btm"]
                bull_poc=nearest_bull["avg"]
            else:
                val.iloc[idx]=fallback_val.iloc[idx]
                bull_poc=fallback_poc.iloc[idx]
                
            bears=active_bear_obs.iloc[idx]
            above_bears=[ob for ob in bears if ob["top"]>=px]
            if above_bears:
                nearest_bear=min(above_bears, key=lambda x: x["top"])
                vah.iloc[idx]=nearest_bear["top"]
                bear_poc=nearest_bear["avg"]
            else:
                vah.iloc[idx]=fallback_vah.iloc[idx]
                bear_poc=fallback_poc.iloc[idx]
                
            if ob_df["os_state"].iloc[idx]==1:
                poc.iloc[idx]=bull_poc
            else:
                poc.iloc[idx]=bear_poc
