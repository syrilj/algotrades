...
def _prob_to_size(p: float, thr: float) -> float:
    ...
    Feedback/AFML lessons baked in:
    - Hard thr=0.6 + step map discards near-threshold edge (v15/v23 gap).
    - Soft continuous map: skip only when p is weak; scale 0.25→1.0 as p rises.
    - Half-Kelly style ramp with payout b≈1.9 (WINNER PLR), floored/capped.
    """
    if not np.isfinite(p):
        return 0.0
    # Soft skip (never as harsh as 0.60 hard gate)
    skip = min(float(thr), 0.52)
    if p < skip:
        return 0.0
    # Full size when high confidence
    full_at = max(skip + 0.18, 0.70)
    if p >= full_at:
        base = 1.0
    else:
        t = (p - skip) / max(full_at - skip, 1e-9)
        base = 0.25 + 0.75 * float(np.clip(t, 0.0, 1.0))
    # Half-Kelly tilt using historical PLR ~1.9
    b = 1.90
    edge = p * b - (1.0 - p)
    if edge <= 0:
        return 0.0
    kelly = 0.5 * edge / b  # typically ~0.15–0.40
    # Blend continuous base with Kelly so high-p trades get full weight
...
def _soft_feature_mult(
    ...
    """Soft size mult from feature-evolve + soft-structure feedback.

    Never hard-blocks. From v36 META_ENSEMBLE + v32 soft_react:
    - atr_pct / range inform vol regime (high ATR → cut)
    - room_pct (space to recent high) → boost when open, cut when tight
    - macd_hist / ret_5d extremes → soft mean-reversion cut (chase)
    - structure_good → ×1.12; chase → ×0.55
    """
    m = 1.0
    ...
    if np.isfinite(macd_hist) and abs(macd_hist) > 0.0:
        # Large hist often exhaustion on this bag (negative IC)
    ...
    if np.isfinite(ret_5d):
        if ret_5d > 0.15:
            m *= 0.55  # vertical spike chase
        elif ret_5d > 0.08:
            m *= 0.80
        elif ret_5d < -0.08:
            m *= 1.05  # mild dip buy context for long primary
    if structure_good:
        m *= 1.12
    if chase:
        m *= 0.55
    return float(np.clip(m, 0.40, 1.25))
...
class SignalEngine:
    """v37_feedback_meta: v23 winner DNA + continuous soft meta size + evolve feature feedback.

    Primary side/rules unchanged (v20b specialists + XLP macro + ARM drop).
    Secondary: same meta-XGB, continuous half-Kelly-ish size, soft feature mults.
    Never hard-blocks on evolve features (v29/v30/v31 failure mode).
    """
    ...
    def __init__(self):
        ...
        # Soft skip; continuous map uses min(thr, 0.52) inside _prob_to_size
        self._thr = float(self._meta_cfg.get("threshold", 0.52))
        ...
        for i in range(len(data)):
            ...
            if not in_pos:
                if bool(long_entry.iloc[i]):
                    ...
                    # Conviction overlay: vol_z + light sector/QQQ RS (soft only)
                    ...
                    # Soft feature/structure mult — never revives a meta zero
                    ...
                    raw = meta_sz * feat_m if meta_sz > 0 else 0.0
