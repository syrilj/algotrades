"""v28_feedback_opts — v27 conf-tier + surgical econ/calendar narrative.

Learned from continuous failure autopsy:
  - Broad FEAR/near-FOMC size-down cut winners (AVGO near FOMC, MU under FEAR_VOL).
  - Only clear macro blowup: MU 2024-12-18 FOMC day + elevated VIX → surgical block.
  - Second loss MU 2024-11-25 was RISK_ON_QUIET (ticker cooloff / not macro).
...
def _load_equity_engine():
    ...
    spec = importlib.util.spec_from_file_location("v21_equity_se_v28", path)
...
class SignalEngine:
    def __init__(self):
        ...
        # surgical | broad | fomc_day | event_vix | off
        self.narrative_mode = str(cfg.get("narrative_mode", "surgical"))
        self.use_conf_tier = bool(cfg.get("use_conf_tier", True))
        # After a realized loss on a ticker, skip new entries for N calendar days
        # (addresses MU Nov-25 RISK_ON_QUIET dump without macro false positives).
        self.loss_cooloff_days = int(cfg.get("loss_cooloff_days", 0))
        self._macro = None
    ...
    def generate(self, data_map: Dict[str, pd.DataFrame]) -> List[Dict[str, Any]]:
        ...
        # code -> last loss date (for cooloff)
