...
def _coulling_score_at_look(df, look: int, vol_sma: int, stop_look: int):
    """Core Coulling flags + raw score at a given look (v39b light negatives)."""
    ...
    spread_ma = spread.rolling(max(10, vol_sma), min_periods=4).mean()
...
def volume_price_state(df, look=3, vol_sma=14, stop_look=5):
    """Coulling VPA dual-speed score (v39c).

    v39b used only fast look=3 (great live react, slight early-window lag).
    v39c blends fast (look=3) + slow (look=5) by vol_regime:
      loud tape → trust fast sensors
      quiet tape → lean slow (less noise)
    Philosophy: react to volume at nodes — never predict price.
    """
    fast = _coulling_score_at_look(df, look=int(look), vol_sma=int(vol_sma), stop_look=int(stop_look))
    slow = _coulling_score_at_look(df, look=5, vol_sma=20, stop_look=8)
    ...
    # Blend weight for fast score: 0.40 quiet → 0.85 loud
    w_fast = (0.40 + 0.30 * vol_regime.clip(0.0, 1.5)).clip(0.40, 0.85)
    vpa_score = (w_fast * fast["score"] + (1.0 - w_fast) * slow["score"]).clip(-2.0, 2.5)

    vpa_ma = vpa_score.rolling(8, min_periods=3).mean()
    vpa_mom = (vpa_score - vpa_ma).fillna(0.0).clip(-1.5, 1.5)

    # Flags from fast path (live reaction); score is dual-speed
    return pd.DataFrame({
        "confirm_up": fast["confirm_up"],
        "red_flag_up": fast["red_flag_up"],
        "healthy_pull": fast["healthy_pull"],
        "dump": fast["dump"],
        "vol_expand": fast["vol_expand"],
        "vol_z": vol_z,
        "vol_regime": vol_regime,
        "no_demand": fast["no_demand"],
        "no_supply": fast["no_supply"],
        "no_supply_test": fast["no_supply_test"],
        "stopping_volume": fast["stopping_volume"],
        "stopping_reclaim": fast["stopping_reclaim"],
        "topping_volume": fast["topping_volume"],
        "effort_anomaly": fast["effort_anomaly"],
        "buying_climax": fast["buying_climax"],
        "climax_recent": fast["climax_recent"],
        "upthrust": fast["upthrust"],
        "spring": fast["spring"],
        "commitment": fast["commitment"],
        "vpa_score": vpa_score.fillna(0.0),
        "vpa_mom": vpa_mom,
        "vpa_fast": fast["score"].fillna(0.0),
        "vpa_slow": slow["score"].fillna(0.0),
    }, index=df.index)
...
def _soft_feature_mult(
    ...
    if demand_node and float(vpa_score) + 0.25 * float(vpa_mom) >= 0.0:
        m *= 1.12
    # Soft DD: expanded ATR + weak/neutral VPA → cut risk (holdout: early lag was mild)
    if (
        np.isfinite(atr_pct)
        and np.isfinite(med_atr_pct)
        and med_atr_pct > 0
        and atr_pct > 1.30 * med_atr_pct
        and float(vpa_score) < 0.30
    ):
        m *= 0.82
    ...
    if high_beta_qqq_weak:
        m *= 0.70
    return float(np.clip(m, 0.35, 1.45))
...
class SignalEngine:
    """v39c_live_tight: dual-speed VPA + live adapt (v39b + holdout fix).

    Same primary SIDE. Secondary:
      - blend fast/slow Coulling score by vol_regime (quiet→slow, loud→fast)
      - soft DD cut when ATR expanded and VPA not supportive
      - vpa_mom + live streak + record_trade()
      - milder mid-trade shrink than v39b (less early-window over-exit)

    Forbidden: climax hard-stacks, price ML primary, hard VPA entry ANDs.
    """
        ...
        for i in range(len(data)):
            ...
            if not in_pos:
                if bool(long_entry.iloc[i]):
                    ...
                    # Live: decaying VPA mom slightly lifts meta confidence
                ...
                soft_exit=False
                # Live-fast soft exits: react when volume story flips
                ...
                if not armed:
                    ...
                    if bool(climax_soft.iloc[i]) and vpa_now < 0.0:
                        soft_exit=True
                    # Rapid adapt exit: VPA collapsing while not yet armed
                    if mom_now <= -0.85 and vpa_now <= -0.35:
                        soft_exit=True
                elif block_red_flag and bool(vp["red_flag_up"].iloc[i]):
                    soft_exit=True
                elif exit_on_sqz_neg and bool(sqz["mom_neg"].iloc[i]) and px<entry_px:
                    soft_exit=True
                elif mom_now <= -1.10 and vpa_now <= -0.60 and px < entry_px:
                    # armed but volume narrative broken against us
                    soft_exit=True
                if hard_stop or trail_stop or soft_exit:
                    ...
                    # Milder mid-trade shrink (v39c): less early-window over-exit
                    if mom_now <= -0.85 and vpa_now < -0.15 and entry_size > 0.25:
