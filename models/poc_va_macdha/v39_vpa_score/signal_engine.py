...
def volume_price_state(df, look=5, vol_sma=20, stop_look=8):
    """Coulling VPA frame + continuous vpa_score (v39).

    Philosophy (react, don't predict price):
      Volume = effort (harder to fake); price = result.
      Score sizes bets; primary SIDE still comes from structure nodes.

    WORKING research only as soft inputs (never hard-block stack):
      + stopping reclaim / no-supply test / spring / confirm_up
      - no_demand / buying climax / effort anomaly / topping / upthrust
    """
    ...
    # Reclaim after absorption (commitment path)
    stopping_reclaim = (stop_recent & green & (ret1 > 0.005) & ~dump).fillna(False)
    # No-supply test after stop (book: buy the test)
    ...
    # Continuous Coulling score — secondary size only
    # Positive = volume supports demand / absorption; negative = trap / distribution
    score = (
        stopping_reclaim.astype(float) * 1.00
        + no_supply_test.astype(float) * 0.70
        + spring.astype(float) * 0.55
        + confirm_up.astype(float) * 0.45
        + healthy_pull.astype(float) * 0.15
        + stopping_volume.astype(float) * 0.20
        - no_demand.astype(float) * 0.75
        - climax_recent.astype(float) * 0.85
        - effort_anomaly.astype(float) * 0.55
        - top_recent.astype(float) * 0.50
        - upthrust.astype(float) * 0.60
        - dump.astype(float) * 0.70
        - red_flag_up.astype(float) * 0.40
    )
    vpa_score = score.clip(-2.5, 2.5)
...
def vpa_score_to_mult(score: float) -> float:
    """Map continuous VPA score → size multiplier (never zero)."""
...
def _soft_feature_mult(
    ...
    """Soft size mult — research stack + continuous VPA. Never hard-blocks.

    Sources:
    - v36 feature evolve: atr/room/macd/ret_5d
    - v32/v35 soft structure: good ×1.15, weak/chase ×0.55
    - v39 continuous vpa_score (replaces binary no_demand/commitment stack)
    - node reaction: demand zone (VAL/POC) test with volume support
    - EMA cloud path: soft path-of-least-resistance, not side predictor
    - high-WR sleeve research: high-beta when QQQ weak → soft cut
    """
    m = 1.0
    ...
    if chase:
        m *= weak_m
    # v39 continuous Coulling score (price can lie; volume less so)
    m *= vpa_score_to_mult(vpa_score)
    # React at demand nodes (volume-profile VAL/POC) when VPA agrees
    if demand_node and float(vpa_score) >= 0.0:
        m *= 1.10
    # EMA cloud: path bias only
    if ema_bull:
        m *= 1.06
    if ema_bear:
        m *= 0.78
    # APLD/IONQ sniper research: QQQ weak → cut high-beta risk
    if high_beta_qqq_weak:
        m *= 0.70
    return float(np.clip(m, 0.30, 1.40))
...
class SignalEngine:
    """v39_vpa_score: react to nodes + continuous VPA, don't predict price.

    Primary SIDE frozen (specialists / POC-VA structure).
    Secondary stack (soft only):
      continuous Coulling vpa_score → size
      demand-node reaction (VAL/POC) when volume agrees
      EMA cloud path context
      v37 continuous soft meta + half-Kelly
      v36 genome risk / streak
      high-beta soft cut when QQQ RS weak

    Forbidden: full climax hard-stacks, price-predict ML, hard VPA entry ANDs.
    """
    ...
    def _signals_on_frame(self, data, cfg, code: str, data_map: Dict[str, pd.DataFrame]):
        ...
        # Conviction overlays + evolve soft features
        vol_z = vp["vol_z"].shift(1).fillna(0.0)
        # Continuous VPA (shift: react to completed bar volume, no look-ahead)
        ...
        # Soft structure: good = in VA lower half + HA green; chase = near VAH + hot macd
        va_mid = (val + vah) / 2.0
        structure_good = (close <= va_mid) & local_ha["ha_green"] & above_vwap
        chase_ob = (close >= vah) & (macd_hist > 0) & (ret_5d > 0.05)
        # Demand nodes = volume-profile supply/demand zones (smart-money inventory)
        # React when price is near VAL/POC (within 0.6 ATR) — not a prediction
        a_safe = atr.replace(0, np.nan)
        near_val = ((close - val).abs() / a_safe) <= 0.60
        near_poc = ((close - poc).abs() / a_safe) <= 0.60
        ...
        # Mild RS conviction boost to proba (feedback: sector/QQQ overlay helped v23)
        ...
        # Genome feedback: scale next entry after win/loss (v36 after_*_mult)
        streak_mult = 1.0
        for i in range(len(data)):
            ...
            if not in_pos:
                if bool(long_entry.iloc[i]):
                    ...
                    # v36 genome risk_pct scale (~1.3×) on secondary only
                    sz = float(np.clip(sz * self._risk_scale, 0.0, 1.0))
                    # Stand aside only when volume screams trap (score floor + meta)
                    if float(vpa_sc.iloc[i]) <= -1.60 and sz > 0:
                        sz = 0.0
                    if sz>0:
                ...
                if not armed:
                    ...
                    # v39: soft exit into recent buying climax (don't chase euphoria) — not a hard entry stack
                    if bool(climax_soft.iloc[i]) and float(vpa_sc.iloc[i]) < 0.0:
                        soft_exit=True
                ...
                if hard_stop or trail_stop or soft_exit:
                    # Update genome streak mult from trade PnL
                    if np.isfinite(entry_px) and entry_px > 0:
                        if px >= entry_px:
                            streak_mult = self._after_win
                        else:
                            streak_mult = self._after_loss
