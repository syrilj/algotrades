...
"""poc_va_macdha multi-variant engine — see MODEL_VARIANT / VARIANT below."""
...
# === VARIANT_START ===
MODEL_VARIANT = "v42_trend_breakout"
VARIANT = {
    "value_area_pct": 0.70,
    "profile_rows": 25,
    "profile_lookback": 20,
...
class SignalEngine:
    def __init__(self):
        v = VARIANT
        self.value_area_pct = v.get("value_area_pct", 0.70)
        self.profile_rows = v.get("profile_rows", 25)
        self.profile_lookback = v.get("profile_lookback", 20)
        self.macd_fast = v.get("macd_fast", 12)
        self.macd_slow = v.get("macd_slow", 26)
        self.macd_signal = v.get("macd_signal", 9)
        self.macd_htf = v.get("macd_htf", "4h")
        self.signal_tf = v.get("signal_tf", "2h")
        self.require_htf_green = v.get("require_htf_green", True)
        self.require_vwap_uptrend = v.get("require_vwap_uptrend", False)
        self.require_above_vwap = v.get("require_above_vwap", False)
        self.require_volume_expand = v.get("require_volume_expand", False)
        self.require_vol_confirm = v.get("require_vol_confirm", False)
        self.block_red_flag = v.get("block_red_flag", False)
        self.block_dump = v.get("block_dump", False)
        self.require_sqz_release = v.get("require_sqz_release", False)
        self.require_mom_pos = v.get("require_mom_pos", False)
        self.require_mom_pos_inc = v.get("require_mom_pos_inc", False)
        self.allow_healthy_pull_entry = v.get("allow_healthy_pull_entry", False)
        self.exit_on_poc_break = v.get("exit_on_poc_break", False)
        self.exit_on_val_break = v.get("exit_on_val_break", False)
        self.exit_below_vwap = v.get("exit_below_vwap", False)
        self.exit_on_sqz_neg = v.get("exit_on_sqz_neg", False)
        self.soft_confidence = v.get("soft_confidence", False)
        self.swing_period = v.get("swing_period", 50)
        self.vol_look = v.get("vol_look", 5)
        self.vol_sma = v.get("vol_sma", 20)
        self.min_confidence = v.get("min_confidence", 0.6)
        self.max_hold_bars = v.get("max_hold_bars", 0)
        self.stop_loss_pct = v.get("stop_loss_pct", 0.0)
        self.trailing_stop_pct = v.get("trailing_stop_pct", 0.0)

    def _confidence(self, parts: dict) -> pd.Series:
        keys = list(parts.keys())
        if not keys:
            return pd.Series(1.0, index=next(iter(parts.values())).index if False else None)
        idx = next(iter(parts.values())).index
        score = pd.Series(0.0, index=idx)
        for s in parts.values():
            score = score + s.astype(float)
    ...
    def _signals_on_frame(self, data):
        ...
        if self.require_volume_expand: gates.append(vol_expand)
        ...
        parts = {
            "poc": poc_ok,
            "va": in_va,
            "ha": htf if self.require_htf_green else pd.Series(True, index=data.index),
            "vwap_dir": uptrend,
            "above_vwap": above_vwap,
            "vol_confirm": vp["confirm_up"] | vp["healthy_pull"],
            "not_red": ~vp["red_flag_up"],
            "mom": sqz["mom_pos"],
            "sqz_off": sqz["sqz_off"] | sqz["sqz_release"],
        }
        conf = self._confidence(parts)

        if self.soft_confidence:
            long_entry = poc_ok & in_va & (conf >= self.min_confidence) & (~vp["red_flag_up"] if self.block_red_flag else True) & (~vp["dump"] if self.block_dump else True)
            if self.require_htf_green:
                long_entry = long_entry & htf
            size = conf.clip(0, 1)
            # map to position: enter at size, else 0
        else:
        ...
        for i in range(len(data)):
            if not in_pos:
                ...
                if self.exit_on_val_break and price < val.iloc[i]: exit_now = True
                if self.exit_below_vwap and pd.notna(vwap.iloc[i]) and price < vwap.iloc[i]: exit_now = True
                ...
                if self.max_hold_bars > 0 and bars_held >= self.max_hold_bars: exit_now = True
                if self.stop_loss_pct > 0 and price < entry_price * (1.0 - self.stop_loss_pct): exit_now = True
                if self.trailing_stop_pct > 0 and price < max_price * (1.0 - self.trailing_stop_pct): exit_now = True
                if exit_now:
                    in_pos = False
                    signal.iloc[i] = 0.0
                else:
