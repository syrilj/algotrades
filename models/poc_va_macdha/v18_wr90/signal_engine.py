...
# v18_wr90: small-cap sleeve + book VPA + vol_z + EMA200 structure (target WR>=90%)
TRADE_UNIVERSE = {"APLD.US", "IONQ.US"}
REGIME_REF = {"QQQ.US"}
REGIME_APPLY_TO = {"APLD.US", "IONQ.US"}
REGIME_GATE = "gate_qqq_trend"
# Extra hard gates for sniper sleeve (toggled by feedback loop)
WR90_REQUIRE_VOL_Z = False
WR90_VOL_Z_MIN = 1.0
WR90_REQUIRE_EMA200 = False
WR90_REQUIRE_COMMITMENT = False
WR90_BLOCK_NO_DEMAND = False
WR90_MIN_CONFIDENCE = 0.55
WR90_VOL_Z_OR_CONFIRM = False
WR90_REQUIRE_DAILY_CONFIRM = False
...
def volume_price_state(df, look=5, vol_sma=20):
    """Volume-price + Coulling VPA extensions for high-WR sleeve."""
    close = df["close"]
    high = df["high"]
    low = df["low"]
    vol = df["volume"]
    ...
    commitment = (confirm_up | stopping_reclaim | reflexive_up).fillna(False)
    ...
    return pd.DataFrame({
        ...
        "no_demand": no_demand.fillna(False),
...
def _daily_vol_z_ok(df: pd.DataFrame, min_z: float = 1.0) -> pd.Series:
    """Prior-day volume z-score gate (no lookahead). Align to intraday index."""
    d = df.copy()
    d.index = pd.to_datetime(d.index)
    if getattr(d.index, "tz", None) is not None:
        d.index = d.index.tz_localize(None)
    daily = d.resample("1D").agg({"close": "last", "volume": "sum"}).dropna()
    ...
    z = ((v - mu) / sd.replace(0, np.nan)).shift(1)  # prior day only
    ok = (z >= float(min_z)).astype(float)
    aligned = ok.reindex(d.index, method="ffill").fillna(0.0)
    return aligned > 0.5


def _daily_confirm_ok(df: pd.DataFrame) -> pd.Series:
    """Prior-day Coulling confirm: up day with volume above 20d SMA (no lookahead)."""
    d = df.copy()
    d.index = pd.to_datetime(d.index)
    if getattr(d.index, "tz", None) is not None:
        d.index = d.index.tz_localize(None)
    ...
    up = c > c.shift(1)
    confirm = (up & (v > vsma)).shift(1)  # prior day only
    confirm = confirm.where(confirm.notna(), False).astype(bool)
    aligned = confirm.astype(float).reindex(d.index, method="ffill").fillna(0.0)
    return aligned > 0.5


def _above_ema200(df: pd.DataFrame) -> pd.Series:
    """Structural bull filter: prior close above EMA200 (trade-desk finding)."""
    c = df["close"].astype(float)
    ema200 = c.ewm(span=200, adjust=False).mean()
    ok = (c > ema200).shift(1)
    return ok.where(ok.notna(), False).astype(bool)
...
class SignalEngine:
    """v18_wr90: sniper sleeve targeting WR>=90%.

    Stacks WORKING findings:
    - v16 universe APLD+IONQ + QQQ trend + vol_expand + block_red_flag
    - Coulling commitment / no-demand (book)
    - vol_z>=1 daily (GEX volume meta)
    - EMA200 structure (trade desk)
    - Raised soft confidence floor
    """
    ...
    def _signals_on_frame(self, data, cfg, regime_ok=None):
        ...
        min_confidence = float(cfg.get("min_confidence", WR90_MIN_CONFIDENCE))
        min_confidence = max(min_confidence, float(WR90_MIN_CONFIDENCE))
        ...
        vol_z_ok = _daily_vol_z_ok(data, WR90_VOL_Z_MIN) if WR90_REQUIRE_VOL_Z else pd.Series(True, index=data.index)
        daily_confirm_ok = _daily_confirm_ok(data)
        ema200_ok = _above_ema200(data) if WR90_REQUIRE_EMA200 else pd.Series(True, index=data.index)
        ...
        if require_vol_confirm:
            gates.append(vp["confirm_up"] | (allow_healthy_pull_entry & vp["healthy_pull"] & above_vwap) | vp["stopping_reclaim"])
        ...
        if WR90_BLOCK_NO_DEMAND: gates.append(~vp["no_demand"])
        if WR90_REQUIRE_COMMITMENT: gates.append(vp["commitment"])
        ...
        # Sniper hard filters (v16 + book + vol_z + structure)
        long_entry = long_entry & (~vp["red_flag_up"].fillna(False).astype(bool))
        long_entry = long_entry & vol_expand.fillna(False)
        if WR90_BLOCK_NO_DEMAND:
            long_entry = long_entry & (~vp["no_demand"].fillna(False).astype(bool))
        if WR90_VOL_Z_OR_CONFIRM:
            # Pass if prior-day vol_z OK OR prior-day Coulling confirm (offline-proven 100% WR path)
            long_entry = long_entry & (
                vol_z_ok.fillna(False).astype(bool) | daily_confirm_ok.fillna(False).astype(bool)
            )
        else:
            if WR90_REQUIRE_COMMITMENT:
                long_entry = long_entry & vp["commitment"].fillna(False).astype(bool)
            if WR90_REQUIRE_VOL_Z:
                long_entry = long_entry & vol_z_ok.fillna(False).astype(bool)
        if WR90_REQUIRE_DAILY_CONFIRM:
            long_entry = long_entry & daily_confirm_ok.fillna(False).astype(bool)
        if WR90_REQUIRE_EMA200:
            long_entry = long_entry & ema200_ok.fillna(False).astype(bool)
