...
"""Universal specialist — family DNA per symbol via tools.specialist_factory.

Not a unique trained agent per ticker. Same VA/VWAP/VPA stack as v65_spec_*;
gates are chosen by family classification (megacap, semi, crypto, default…).
"""
...
def _family_cfg_for_code(code: str) -> Dict[str, Any]:
    """Resolve family DNA gates for one symbol. Falls back to defaults."""
    cfg = dict(_DEFAULTS)
    try:
        import specialist_factory as sf  # type: ignore

        info = sf.classify_symbol(code)
        gates = info.get("gates") or {}
        for k, v in gates.items():
            if k in _GATE_KEYS:
                cfg[k] = v
        cfg["_family"] = info.get("family")
        cfg["_specialist"] = info.get("specialist")
        cfg["_prior"] = info.get("prior")
    except Exception:
        cfg["_family"] = "default_equity"
        cfg["_specialist"] = None
        cfg["_prior"] = 0.55
    return cfg
...
class SignalEngine:
    def __init__(self, **kwargs):
        ...
        self.last_classification: Dict[str, Any] = {}
    ...
    def _signals_on_frame(self, data):
        ...
        for i in range(len(data)):
            if not in_pos:
                ...
                exit_now = False
                if self.require_htf_green and not bool(htf.iloc[i]):
                    exit_now = True
                if self.exit_on_poc_break and not bool(poc_ok.iloc[i]):
                    exit_now = True
                ...
                if self.exit_below_vwap and pd.notna(vwap.iloc[i]) and close.iloc[i] < vwap.iloc[i]:
                    exit_now = True
                if self.exit_on_sqz_neg and bool(sqz["mom_neg"].iloc[i]):
                    exit_now = True
                if bool(local_ha["ha_red"].iloc[i]) and not bool(htf.iloc[i]):
                    exit_now = True
                if self.block_red_flag and bool(vp["red_flag_up"].iloc[i]):
                    exit_now = True
                if exit_now:
                    in_pos = False
                    signal.iloc[i] = 0.0
    ...
    def _one(self, df, code: str | None = None):
        # Per-symbol family DNA (universal path)
        if code:
            fam_cfg = _family_cfg_for_code(code)
            self._apply(fam_cfg)
            self.last_classification[code] = {
                "family": fam_cfg.get("_family"),
                "specialist": fam_cfg.get("_specialist"),
                "prior": fam_cfg.get("_prior"),
            }
        data = df.copy()
    ...
    def generate(self, data_map):
        return {c: self._one(df, code=c) for c, df in data_map.items()}

    def classify(self, code: str) -> Dict[str, Any]:
        return _family_cfg_for_code(code)
