...
# --- v20 book construction (from WORKING findings) ---
TRADE_DROP = {"ARM.US"}  # consistently worst DNA / choppy losers
REGIME_FLAT = {"XLP.US", "QQQ.US", "AAPL.US", "MSFT.US", "NVDA.US", "GOOGL.US", "AMZN.US", "META.US"}
...
def _xlp_spy_allow(xlp_df: pd.DataFrame, spy_df: pd.DataFrame) -> pd.Series:
    """Block defensive XLP/SPY uptrends; allow risk-on or neutral. Causal (shifted)."""
    ...
    allow = (~defensive).astype(bool)
    return allow.shift(1).fillna(False)


def _qqq_trend_ok(qqq_df: pd.DataFrame) -> pd.Series:
    c = _daily_close(qqq_df)
    macd = _ema(c, 12) - _ema(c, 26)
    hist = macd - _ema(macd, 9)
    green = ((hist > 0) & (hist > hist.shift(1))).shift(1).fillna(False).astype(bool)
    above = (c > c.rolling(20, min_periods=20).mean()).shift(1).fillna(False).astype(bool)
...
def _mag7_breadth_ge4(data_map: Dict[str, pd.DataFrame], ref_index) -> pd.Series:
    ...
    for code in mag7:
        ...
        above = (daily["close"] > daily["close"].rolling(20, min_periods=20).mean()).shift(1).fillna(False)
        parts.append(above.reindex(ref_index).ffill().fillna(False))
    if not parts:
        return pd.Series(True, index=ref_index)  # fail-open if Mag7 missing
    return (pd.concat(parts, axis=1).sum(axis=1) >= 4).astype(bool)
...
class SignalEngine:
    """v20: v16 meta+Kelly + XLP/SPY defensive block + dual-sleeve QQQ/Mag7; drop ARM."""
        ...
        for i in range(len(data)):
            ...
            if not in_pos:
                if bool(long_entry.iloc[i]):
                    ...
                    meta_sz=_prob_to_size(proba,self._thr)
                    # Meta gate wins: zero stays zero; only scale positive meta sizes
    ...
    def generate(self, data_map: Dict[str, pd.DataFrame]) -> Dict[str, pd.Series]:
        ...
        # Macro: block defensive XLP/SPY (v19 diagnosis)
        ...
        qqq_df = data_map.get("QQQ.US")
        qqq_ok = _qqq_trend_ok(qqq_df) if qqq_df is not None and not qqq_df.empty else None

        # Mag7 breadth on daily calendar
        ref_idx = None
        if spy_df is not None:
            ref_idx = _daily_close(spy_df).index
        mag7_ok = _mag7_breadth_ge4(data_map, ref_idx) if ref_idx is not None else None
        ...
        for code, df in data_map.items():
            if code in REGIME_FLAT or code in TRADE_DROP:
                ...
                continue
            sig = self._one(code, df)
            # Align daily gates onto signal index
            idx = pd.to_datetime(sig.index)
            if getattr(idx, "tz", None) is not None:
                idx = idx.tz_localize(None)
            days = idx.normalize()

            gate = pd.Series(True, index=sig.index)
            if macro_allow is not None:
                g = macro_allow.reindex(days).ffill().fillna(False)
                g.index = sig.index
                gate = gate & g.astype(bool)
            if code in SNIPER and qqq_ok is not None:
                g = qqq_ok.reindex(days).ffill().fillna(False)
                g.index = sig.index
                gate = gate & g.astype(bool)
            if code in LARGE:
                if qqq_ok is not None:
                    ...
                    g.index = sig.index
                    gate = gate & g.astype(bool)
                if mag7_ok is not None:
                    g = mag7_ok.reindex(days).ffill().fillna(False)
                    g.index = sig.index
                    gate = gate & g.astype(bool)
            out[code] = (sig.where(gate, 0.0)).astype(float)
        return out
