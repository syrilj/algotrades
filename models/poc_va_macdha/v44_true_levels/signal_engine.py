...
def calculate_lonesome_volume_profile(df: pd.DataFrame, lookback: int = 150, rows: int = 24, value_area_pct: float = 70.0) -> pd.DataFrame:
    n = len(df)
    ...
    denom = 2.0 * topwick + 2.0 * bottomwick + body
    body_vol = np.zeros(n)
    topwick_vol = np.zeros(n)
    bottomwick_vol = np.zeros(n)
    
    valid = denom > 0
    body_vol[valid] = body[valid] * volume[valid] / denom[valid]
    topwick_vol[valid] = 2.0 * topwick[valid] * volume[valid] / denom[valid]
    bottomwick_vol[valid] = 2.0 * bottomwick[valid] * volume[valid] / denom[valid]
    body_vol[~valid] = volume[~valid]
    
    def get_overlap(y1_min, y1_max, y2_min, y2_max):
        return np.maximum(0.0, np.minimum(y1_max, y2_max) - np.maximum(y1_min, y2_min))
        
    for i in range(lookback, n):
        sl = slice(i - lookback, i)
        top_val = np.max(high[sl])
        bot_val = np.min(low[sl])
        
        if not np.isfinite(top_val) or not np.isfinite(bot_val) or top_val <= bot_val:
            continue
            
        step = (top_val - bot_val) / rows
        levels = bot_val + step * np.arange(rows + 1)
        
        total_vols = np.zeros(rows)
        
        w_body_top = body_top[sl]
        w_body_bot = body_bot[sl]
        w_topwick = topwick[sl]
        w_bottomwick = bottomwick[sl]
        w_body = body[sl]
        w_high = high[sl]
        w_low = low[sl]
        
        w_body_vol = body_vol[sl]
        w_topwick_vol = topwick_vol[sl]
        w_bottomwick_vol = bottomwick_vol[sl]
        
        for r in range(rows):
            b_min = levels[r]
            b_max = levels[r + 1]
            
            overlap_body = get_overlap(b_min, b_max, w_body_bot, w_body_top)
            body_h = w_body
            vol_from_body = np.zeros(len(w_body))
            h_valid = body_h > 0
            vol_from_body[h_valid] = overlap_body[h_valid] * w_body_vol[h_valid] / body_h[h_valid]
            vol_from_body[~h_valid] = w_body_vol[~h_valid] * ((b_min <= w_body_bot[~h_valid]) & (w_body_bot[~h_valid] < b_max))
            
            overlap_top = get_overlap(b_min, b_max, w_body_top, w_high)
            top_h = w_topwick
            vol_from_top = np.zeros(len(w_topwick))
            t_valid = top_h > 0
            vol_from_top[t_valid] = overlap_top[t_valid] * w_topwick_vol[t_valid] / top_h[t_valid]
            
            overlap_bot = get_overlap(b_min, b_max, w_low, w_body_bot)
            bot_h = w_bottomwick
            vol_from_bot = np.zeros(len(w_bottomwick))
            b_valid = bot_h > 0
            vol_from_bot[b_valid] = overlap_bot[b_valid] * w_bottomwick_vol[b_valid] / bot_h[b_valid]
            
            total_vols[r] = np.sum(vol_from_body + vol_from_top / 2.0 + vol_from_bot / 2.0)
            
        poc_idx = int(np.argmax(total_vols))
        total_vol = total_vols.sum()
        if total_vol <= 0:
            continue
            
        target_vol = total_vol * value_area_pct / 100.0
        va_vol = total_vols[poc_idx]
        
        up_idx = poc_idx
        dn_idx = poc_idx
        
        while va_vol < target_vol:
            if up_idx == rows - 1 and dn_idx == 0:
                break
            up_vol = total_vols[up_idx + 1] if up_idx < rows - 1 else 0.0
            dn_vol = total_vols[dn_idx - 1] if dn_idx > 0 else 0.0
        ...
        poc[i] = (levels[poc_idx] + levels[poc_idx + 1]) / 2.0
        vah[i] = levels[up_idx + 1]
        val[i] = levels[dn_idx]
...
class SignalEngine:
    """v44_true_levels: v39d baseline + LTB Volume Profile + Volume Z entry gate.
    ...
    def __init__(self):
        ...
        try:
            ...
            self._live_streak = float(_la.size_mult_for("v44_true_levels") or 1.0)
    ...
    def _signals_on_frame(self, data, cfg, code: str, data_map: Dict[str, pd.DataFrame]):
        ...
        require_volume_z=bool(cfg.get("require_volume_z",False)); vol_z_threshold=float(cfg.get("vol_z_threshold",1.0))
        
        ltb_va = value_area_pct * 100.0 if value_area_pct <= 1.0 else value_area_pct
        levels = calculate_lonesome_volume_profile(data, profile_lookback, profile_rows, ltb_va)
        ...
        if require_volume_z: gates.append(vol_z >= vol_z_threshold)
        long_hard=gates[0]
        ...
        if soft_confidence:
            ...
            if require_volume_z: long_entry=long_entry&(vol_z >= vol_z_threshold)
        else:
