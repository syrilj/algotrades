...
TRADE_UNIVERSE = {"TSLA.US", "ARM.US", "MU.US", "SPY.US", "IONQ.US", "APLD.US"}
REGIME_REF = {"QQQ.US"}
REGIME_APPLY_TO = {"TSLA.US", "ARM.US", "MU.US", "IONQ.US", "APLD.US"}
...
class SignalEngine:
    ...
    def generate(self, data_map: Dict[str, pd.DataFrame]) -> Dict[str, pd.Series]:
        ...
        for code, df in data_map.items():
            if code in REGIME_REF:
                out[code] = pd.Series(0.0, index=df.index)
                continue
            if code not in TRADE_UNIVERSE and code not in self.routing:
                out[code] = pd.Series(0.0, index=df.index)
                continue
