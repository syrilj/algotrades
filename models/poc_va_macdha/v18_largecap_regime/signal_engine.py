...
# v18 large-cap sleeve — Mag7/QQQ DNA (from multiloop Loop B/D)
# Universe: TSLA + MU only. Gate: QQQ HTF green ∧ Mag7 breadth ≥4 + block red-flag.
# Do not reuse APLD sniper filters on these names.

TRADE_UNIVERSE = {"TSLA.US", "MU.US"}
REGIME_REF = {"QQQ.US"}
MAG7 = ["AAPL.US", "MSFT.US", "NVDA.US", "GOOGL.US", "AMZN.US", "META.US", "TSLA.US"]
REGIME_APPLY_TO = {"TSLA.US", "MU.US"}
REGIME_GATE = "gate_qqq_and_mag7"


def _qqq_htf_green(qqq: pd.DataFrame) -> pd.Series:
    df = qqq.copy()
    ...
    return green.where(green.notna(), False).astype(bool)


def _qqq_trend_ok(qqq: pd.DataFrame) -> pd.Series:
    """Kept for compatibility; large-cap sleeve uses qqq_and_mag7 in generate()."""
    df = qqq.copy()
    df.index = pd.to_datetime(df.index)
    if getattr(df.index, "tz", None) is not None:
        df.index = df.index.tz_localize(None)
...
class SignalEngine:
    """v15 regime: v14 risk-kelly + OOS-proven QQQ trend gate (non-SPY).
    ...
    def generate(self, data_map: Dict[str, pd.DataFrame]) -> Dict[str, pd.Series]:
        ...
        if "QQQ.US" in data_map:
            ...
            qqq_green = _qqq_htf_green(daily)
            mag7 = _mag7_breadth_ge4(data_map, daily.index)
            regime_ok = (qqq_green & mag7).astype(bool)
        out = {}
