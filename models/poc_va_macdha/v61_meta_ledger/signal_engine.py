...
def _load_secondary_meta(model_dir: Path) -> Optional[dict]:
    """Secondary logistic meta-labeler (train-window fit only). JSON weights."""
    path = model_dir / "secondary_meta.json"
    if not path.exists():
        return None
    try:
...
def _secondary_prob(sec: dict, feats: list[float], meta_proba: float, adj_proba: float) -> float:
    """P(win) from standardized ledger features + primary meta probs.

    ``feat_cols`` order from training: ``f_*`` (aligned with primary feat order)
    then ``meta_proba``, ``adj_proba``.
    """
    cols = list(sec.get("feat_cols") or [])
    mean = np.asarray(sec.get("mean") or [], dtype=float)
    scale = np.asarray(sec.get("scale") or [], dtype=float)
    coef = np.asarray(sec.get("coef") or [], dtype=float)
    intercept = float(sec.get("intercept", 0.0))
    f_cols = [c for c in cols if c.startswith("f_")]
    values: list[float] = []
    for c in cols:
        if c == "meta_proba":
            values.append(float(meta_proba))
        elif c == "adj_proba":
            values.append(float(adj_proba))
        elif c.startswith("f_"):
            j = f_cols.index(c)
            values.append(float(feats[j]) if j < len(feats) else 0.0)
        else:
            values.append(0.0)
...
def _secondary_size_mult(sec: dict, p: float) -> float:
    """Soft size map: skip below p_skip, linear to full at p_full."""
    p_skip = float(sec.get("p_skip", 0.45))
    p_full = float(sec.get("p_full", 0.65))
    if p < p_skip:
        return 0.0
    if p_full <= p_skip:
        return 1.0
    return float(np.clip((p - p_skip) / (p_full - p_skip), 0.0, 1.0))


class SignalEngine:
    """v61_meta_ledger: v39d_confluence + secondary logistic soft-size meta-label.

    Primary side/entries unchanged from v39d. A train-only logistic on ledger
    features scales (or skips) size. Exits unchanged.
    """
    ...
    def __init__(self):
        ...
        self._secondary = _load_secondary_meta(model_dir)
        ...
        for i in range(len(data)):
            ...
            if not in_pos:
                if bool(long_entry.iloc[i]):
                    ...
                    # Secondary meta-label soft size (Bet 1). No-op if artifact missing.
                    if self._secondary is not None and sz > 0:
                        p2 = _secondary_prob(self._secondary, feats, proba, adj_proba)
                        sz = float(sz * _secondary_size_mult(self._secondary, p2))
