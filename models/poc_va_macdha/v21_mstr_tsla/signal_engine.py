...
class SignalEngine:
    """v21 mstr_tsla: MSTR+TSLA focus, compound equity, higher risk with account floor.

    LSE risk lessons applied:
    - Position sizing > entries (Kelly / half-Kelly / vol-adjust)
    - Fixed-fractional risk to stop (risk_pct)
    - Cut losers fast (hard ATR stop)
    - Let winners run (ATR trail after arm)
    - Drawdowns are asymmetric — soft-cut / halt / flatten on portfolio DD
    """

    def __init__(self):
        self.routing = _ROUTING
        self.risk_pct = 0.01
        # Allow more risk than v20, but never melt the account.
        self.portfolio_dd = {
            "soft_pct": 0.08,
            "soft_mult": 0.65,
            "halt_entries_pct": 0.20,
            "flatten_pct": 0.35,
        }
        self.kelly_fraction_default = 0.75  # more aggressive than half-Kelly

    def _cfg(self, code):
        # MSTR uses TSLA high-beta DNA (no dedicated specialist yet).
        if code == "MSTR.US" and code not in self.routing:
            return dict(self.routing.get("TSLA.US", self.routing.get("SPY.US", {})))
        return self.routing.get(code, self.routing.get("TSLA.US", {}))
    ...
    def _signals_on_frame(self, data, cfg):
        ...
        kelly_fraction = float(cfg.get("kelly_fraction", getattr(self, "kelly_fraction_default", 0.5)))
        risk_pct = float(cfg.get("risk_pct", self.risk_pct))
        ...
        for i in range(len(data)):
            ...
            if k <= 0:
                size.iloc[i] = 0.0
                continue
            # Optional fixed-fractional dampener (default off: risk_pct<=0 or >=1%).
            # When risk_pct < 1%, scale vs a 3% reference stop; floor 0.5 to avoid over-shrink.
            if risk_pct > 0 and risk_pct < 0.01:
                ap = float(atr_pct.iloc[i]) if np.isfinite(atr_pct.iloc[i]) else med_atr_pct
                stop_frac = max(stop_atr * max(ap, 1e-6), 1e-6)
                ff = float(np.clip(risk_pct / stop_frac, 0.5, 1.0))
                size.iloc[i] = float(np.clip(k * ff, 0.0, 1.0))
            else:
                size.iloc[i] = float(k)
    ...
    def _portfolio_dd_overlay(
        self,
        signal_map: Dict[str, pd.Series],
        data_map: Dict[str, pd.DataFrame],
    ) -> Dict[str, pd.Series]:
        """Scale/halt/flatten new exposure from a shared mark-to-market equity curve."""
        dd = self.portfolio_dd
        soft_pct = float(dd.get("soft_pct", 0.05))
        soft_mult = float(dd.get("soft_mult", 0.5))
        halt_pct = float(dd.get("halt_entries_pct", 0.10))
        flatten_pct = float(dd.get("flatten_pct", 0.15))
        ...
        aligned_sig = {
            code: signal_map[code].reindex(idx, method="ffill").fillna(0.0)
            for code in closes
        }
        aligned_px = {
            code: closes[code].reindex(idx, method="ffill")
            for code in closes
        }
        out = {code: aligned_sig[code].copy() for code in closes}
        n = max(len(closes), 1)
        sleeve = 1.0 / n
        equity = 1.0
        peak = 1.0
        prev_w = {code: 0.0 for code in closes}
        prev_px = {code: float("nan") for code in closes}

        for ts in idx:
            # Mark-to-market prior weights.
            ret = 0.0
            for code in closes:
                px = aligned_px[code].loc[ts]
                if not np.isfinite(px) or px <= 0:
                    continue
                pp = prev_px[code]
                if np.isfinite(pp) and pp > 0 and prev_w[code] != 0:
                    ret += prev_w[code] * ((float(px) / pp) - 1.0)
                prev_px[code] = float(px)
            equity *= 1.0 + ret
            peak = max(peak, equity)
            dd_now = (peak - equity) / peak if peak > 0 else 0.0

            if dd_now >= flatten_pct:
                for code in closes:
                    out[code].loc[ts] = 0.0
                    prev_w[code] = 0.0
                continue
            ...
            if dd_now >= soft_pct:
                mult = 0.0 if dd_now >= halt_pct else soft_mult
            else:
                mult = 1.0

            for code in closes:
                raw = float(aligned_sig[code].loc[ts])
                was = prev_w[code]
                if raw <= 0:
                    new_w = 0.0
                elif was <= 0:
                    # New entry — apply DD throttle / halt.
                    new_w = 0.0 if mult <= 0 else raw * sleeve * mult
                else:
                    # Hold existing (allow size shrink from engine exits).
                    new_w = min(was, raw * sleeve) if mult < 1 else raw * sleeve
                    if mult <= 0:
                        new_w = was  # halt new risk only; keep open until engine exits
                # Represent as per-symbol signal in [0,1] (undo sleeve for engine scale).
                out[code].loc[ts] = float(np.clip(new_w / sleeve if sleeve else 0.0, 0.0, 1.0))
                prev_w[code] = new_w

        # Reindex back to each symbol's original signal index.
        result: Dict[str, pd.Series] = {}
        for code, series in signal_map.items():
            if code in out:
                result[code] = out[code].reindex(series.index, method="ffill").fillna(0.0)
            else:
                result[code] = series
        return result

    def generate(self, data_map: Dict[str, pd.DataFrame]) -> Dict[str, pd.Series]:
        base = {code: self._one(code, df) for code, df in data_map.items()}
        return self._portfolio_dd_overlay(base, data_map)
