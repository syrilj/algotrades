...
class SignalEngine:
    """v39b_live_adapt: react to nodes + fast VPA for live trading.

    Same primary SIDE as v38/v39. Secondary adapts rapidly:
      - shorter VPA lookbacks
      - vpa_mom + vol_regime scale size every bar
      - live streak that stacks wins/losses then mean-reverts
      - external record_trade() for desk/paper feedback between sessions
      - no hard stand-aside floor (v39 capacity kill)
        ...
        for i in range(len(data)):
            ...
            if not in_pos:
                ...
                if hard_stop or trail_stop or soft_exit:
                    ...
                    # Scale open position down if VPA flips hard mid-trade (live risk)
                    if mom_now <= -0.70 and vpa_now < 0 and entry_size > 0.25:
                        entry_size = max(0.25, entry_size * 0.85)
