...
def order_flow_state(df, lookback=100, short_len=20, mid_len=50, long_len=100, small_pct=0.75, medium_pct=0.90, big_pct=0.97):
    """OHLCV-safe order flow, absorption, volume cluster and CVD read.

    Extracts the usable parts of the Pine OrderFlow Absorption Matrix script:
      - candle-delta as a proxy for LTF delta (no sub-hour data available)
      - wick absorption volume
      - volume/ delta percentile clusters (small / medium / whale)
      - cumulative volume delta (CVD) bias
      - buy/sell volume imbalance
      - composite flow score (imbalance + CVD bias + absorption bias + last cluster)

    All quantile thresholds are point-in-time (shifted) and rolling sums use the
    current bar and prior ``lookback-1`` bars, matching the v39b live-sensor style.
    """
    ...
    rng_safe = rng.fillna(1e-12)

    # candle delta proxy for LTF delta
    candle_delta = pd.Series(np.where(close > open_, vol, np.where(close < open_, -vol, 0.0)), index=df.index).fillna(0.0)
    abs_delta = candle_delta.abs()

    # wick absorption: allocate volume to the wick in the direction of the move
    upper_wick = (high - np.maximum(open_, close)).clip(lower=0).fillna(0.0)
    lower_wick = (np.minimum(open_, close) - low).clip(lower=0).fillna(0.0)
    abs_upper = pd.Series(np.where(close > open_, vol * (upper_wick / rng_safe), 0.0), index=df.index).fillna(0.0)
    abs_lower = pd.Series(np.where(close < open_, vol * (lower_wick / rng_safe), 0.0), index=df.index).fillna(0.0)
    abs_total = (abs_upper + abs_lower).fillna(0.0)

    def _pct(s, win, q):
        return s.rolling(win, min_periods=max(1, win // 2)).quantile(q).shift(1)

    # volume percentile thresholds (small / medium / big) across three windows
    vS_s = _pct(vol, short_len, small_pct); vS_m = _pct(vol, mid_len, small_pct); vS_l = _pct(vol, long_len, small_pct)
    vM_s = _pct(vol, short_len, medium_pct); vM_m = _pct(vol, mid_len, medium_pct); vM_l = _pct(vol, long_len, medium_pct)
    vB_s = _pct(vol, short_len, big_pct); vB_m = _pct(vol, mid_len, big_pct); vB_l = _pct(vol, long_len, big_pct)

    # delta percentile thresholds
    dS_s = _pct(abs_delta, short_len, small_pct); dS_m = _pct(abs_delta, mid_len, small_pct); dS_l = _pct(abs_delta, long_len, small_pct)
    dM_s = _pct(abs_delta, short_len, medium_pct); dM_m = _pct(abs_delta, mid_len, medium_pct); dM_l = _pct(abs_delta, long_len, medium_pct)
    dB_s = _pct(abs_delta, short_len, big_pct); dB_m = _pct(abs_delta, mid_len, big_pct); dB_l = _pct(abs_delta, long_len, big_pct)

    def _consensus(ps, pm, pl):
        return (ps.astype(int) + pm.astype(int) + pl.astype(int)) >= 2

    vol_big = _consensus(vol >= vB_s, vol >= vB_m, vol >= vB_l)
    vol_med = (~vol_big) & _consensus(vol >= vM_s, vol >= vM_m, vol >= vM_l)
    vol_small = (~vol_big) & (~vol_med) & _consensus(vol >= vS_s, vol >= vS_m, vol >= vS_l)

    d_big = _consensus(abs_delta >= dB_s, abs_delta >= dB_m, abs_delta >= dB_l)
    d_med = (~d_big) & _consensus(abs_delta >= dM_s, abs_delta >= dM_m, abs_delta >= dM_l)
    d_small = (~d_big) & (~d_med) & _consensus(abs_delta >= dS_s, abs_delta >= dS_m, abs_delta >= dS_l)

    # Volume OR Delta (most inclusive) for clusters
    is_big = vol_big | d_big
    is_med = (~is_big) & (vol_med | d_med)
    is_small = (~is_big) & (~is_med) & (vol_small | d_small)
    any_cluster = is_big | is_med | is_small

    flow_side = pd.Series(np.where(close > open_, 1, np.where(close < open_, -1, 0)), index=df.index)
    vol_tier = np.where(is_big, 3, np.where(is_med, 2, np.where(is_small, 1, 0)))

    # rolling cumulative metrics
    min_lb = max(1, lookback // 4)
    cvd = candle_delta.rolling(lookback, min_periods=min_lb).sum()
    abs_cvd = abs_delta.rolling(lookback, min_periods=min_lb).sum().replace(0, np.nan)
    cvd_bias = (cvd / abs_cvd).fillna(0.0)

    buy_vol = np.where(close > open_, vol, 0.0)
    sell_vol = np.where(close < open_, vol, 0.0)
    # short-term imbalance vs long-term CVD bias so they are not identical
    imbalance_lb = max(5, lookback // 10)
    imbalance_min = max(1, imbalance_lb // 4)
    tot_vol = vol.rolling(imbalance_lb, min_periods=imbalance_min).sum().replace(0, np.nan)
    imbalance = ((pd.Series(buy_vol, index=df.index).rolling(imbalance_lb, min_periods=imbalance_min).sum() - pd.Series(sell_vol, index=df.index).rolling(imbalance_lb, min_periods=imbalance_min).sum()) / tot_vol).fillna(0.0)

    abs_upper_sum = abs_upper.rolling(lookback, min_periods=min_lb).sum()
    abs_lower_sum = abs_lower.rolling(lookback, min_periods=min_lb).sum()
    abs_total_sum = (abs_upper_sum + abs_lower_sum).replace(0, np.nan)
    absorption_bias = ((abs_lower_sum - abs_upper_sum) / abs_total_sum).fillna(0.0)

    # last cluster direction (current if any cluster, else previous)
    last_cluster_dir = flow_side.where(any_cluster).ffill().fillna(0.0)

    # Pine-style composite flow score
    score = ((imbalance * 0.32 + cvd_bias * 0.30 + absorption_bias * 0.23 + last_cluster_dir * 0.15) * 100.0).clip(-100.0, 100.0).fillna(0.0)
    ...
    return pd.DataFrame({
        ...
        "imbalance": imbalance,
        "vol_tier": pd.Series(vol_tier, index=df.index, dtype=float),
        "flow_side": flow_side.astype(float),
        "last_cluster_dir": last_cluster_dir.astype(float),
        "flow_score": score,
    }, index=df.index)
...
class SignalEngine:
    """v44_absorption: v39d_confluence + OHLCV-safe order-flow/delta/absorption overlay.

    Keeps v39d live VPA stack and adds:
      - candle-delta / CVD bias / absorption bias / imbalance features
      - volume/delta percentile clusters
      - composite flow score into the XGB meta classifier and adj_proba sizing
    ...
    def __init__(self):
        ...
        try:
            ...
            self._live_streak = float(_la.size_mult_for("v44_absorption") or 1.0)
            # prefer model-specific if any history under v39d/v39b lineage
            m39d = float(_la.size_mult_for("v39d_confluence") or 1.0)
            m39b = float(_la.size_mult_for("v39b_live_adapt") or 1.0)
            self._live_streak = float(np.clip(0.5 * self._live_streak + 0.25 * m39d + 0.25 * m39b, 0.45, 1.45))
        except Exception:
            pass
    ...
    def _meta_row(self, code, i, close, poc, val, vwap, atr, local_ha, macd_hist, above_vwap, vp, of, htf, conf, spy_reg):
        ...
        row = {
            ...
            "flow_score": float(of["flow_score"].iloc[i]) if pd.notna(of["flow_score"].iloc[i]) else 0.0,
            "cvd_bias": float(of["cvd_bias"].iloc[i]) if pd.notna(of["cvd_bias"].iloc[i]) else 0.0,
            "absorption_bias": float(of["absorption_bias"].iloc[i]) if pd.notna(of["absorption_bias"].iloc[i]) else 0.0,
            "imbalance": float(of["imbalance"].iloc[i]) if pd.notna(of["imbalance"].iloc[i]) else 0.0,
        }
    ...
    def _signals_on_frame(self, data, cfg, code: str, data_map: Dict[str, pd.DataFrame]):
        ...
        of=order_flow_state(data, 100).shift(1).fillna(0.0)
        ...
        for i in range(len(data)):
            ...
            if not in_pos:
                if bool(long_entry.iloc[i]):
                    feats=self._meta_row(code,i,close,poc,val,vwap,atr,local_ha,macd_hist,above_vwap,vp,of,htf,conf,spy_reg)
                    ...
                    if (
                        ...
                        boost += 0.02
                    # Order-flow score is an XGB feature only; do not double-count in adj_proba
                    ...
                    # No hard stand-aside floor (v39 ablation A) — only soft size
                    if sz>0:
