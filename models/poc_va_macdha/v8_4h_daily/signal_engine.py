...
class SignalEngine:
    def __init__(
        ...
        self.block_red_flag = block_red_flag
        self.block_dump = block_dump
        self.require_sqz_release = require_sqz_release
        self.require_mom_pos = require_mom_pos
        self.require_mom_pos_inc = require_mom_pos_inc
        self.allow_healthy_pull_entry = allow_healthy_pull_entry
        self.exit_on_poc_break = exit_on_poc_break
        self.exit_on_val_break = exit_on_val_break
        self.exit_below_vwap = exit_below_vwap
        self.exit_on_sqz_neg = exit_on_sqz_neg
        self.soft_confidence = soft_confidence
        self.swing_period = swing_period
        self.vol_look = vol_look
        self.vol_sma = vol_sma
        self.min_confidence = min_confidence

    def _confidence(self, parts):
        idx = next(iter(parts.values())).index
        score = sum((s.astype(float) for s in parts.values()))
        # sum of series
        total = None
        for s in parts.values():
            total = s.astype(float) if total is None else total + s.astype(float)
    ...
    def _signals_on_frame(self, data):
        ...
        parts = {
            "poc": poc_ok, "va": in_va, "ha": htf,
            "vwap_dir": uptrend, "above_vwap": above_vwap,
            "vol_confirm": vp["confirm_up"] | vp["healthy_pull"],
            "not_red": ~vp["red_flag_up"], "mom": sqz["mom_pos"],
        ...
        for i in range(len(data)):
            if not in_pos:
                ...
                if exit_now:
                    in_pos = False; signal.iloc[i] = 0.0
                else:
                    signal.iloc[i] = float(size.iloc[i]) if self.soft_confidence else 1.0
                    in_pos = True
